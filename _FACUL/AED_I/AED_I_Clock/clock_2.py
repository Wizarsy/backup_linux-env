...
def pol_np(points, angle, cx, cy, b):
  sin = math.sin(angle * pi / b )
  cos = math.cos(angle * pi / b )
  np = list(points)
  for p in range(len(list(np))):
    np[p] = list(np[p])
    x = np[p][0] - cx
    y = np[p][1] - cy
    np[p][0] = cx + (x * cos - y * sin)
    np[p][1] = cy + (x * sin + y * cos)
  return np
...
while True:
  ...
  nph = pol_np(hour_arrow_coord, gethour(), clock_center, clock_center, 6)
  hour_arrow = Polygon(Point(nph[0][1], nph[0][1]), Point(nph[1][0], nph[1][1]), Point(nph[2][0], nph[2][1])).draw(win)
  hour_arrow.setOutline('Black')
  hour_arrow.setFill('Black')
  hour_arrow.setWidth(2)

  npm = pol_np(minute_arrow_coord, getminute(), clock_center, clock_center, 30)
  minute_arrow = Polygon(Point(npm[0][0], npm[0][1]), Point(npm[1][0], npm[1][1]), Point(npm[2][0], npm[2][1])).draw(win)
  minute_arrow.setOutline('Black')
  minute_arrow.setFill('Black')
  minute_arrow.setWidth(2)

  nps = pol_np(second_arrow_coord, getsecond(), clock_center, clock_center, 30)
  second_arrow = Polygon(Point(nps[0][0], nps[0][1]), Point(nps[1][0], nps[1][1]), Point(nps[2][0], nps[2][1])).draw(win)
